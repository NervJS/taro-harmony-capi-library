...
namespace TaroDOM {
    ...
    void TaroImageNode::repairSizeIfNeed() {
        ...
        if (std::isnan(image_raw_width) || std::isnan(image_raw_height)) return;
        if (layoutDiffer_.GetDisplayStyle() == YGDisplayNone) return;
        if (!is_first_layout_finish_) return;
        ...
        if (mode_ == "scaleToFill") {
            ...
            if (cw == 0) {
                if (style_ref_->width.has_value() && style_ref_->width.value().Unit() == DimensionUnit::CALC) return;
                SetWidth(image_raw_width);
            }
            if (ch == 0) {
                if (style_ref_->height.has_value() && style_ref_->height.value().Unit() == DimensionUnit::CALC) return;
